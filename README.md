...
### 方式 A：直接打开（✅ 零依赖，2026-09-18 修复）

把仓库下载/解压到本地，双击 `index.html` 即可用，**不需要任何服务器**。整站是纯静态的，这也是「零上传」最硬的那条路：不联网、不起服务、数据连进程边界都不出。

> 2026-09-18 之前这条路是坏的：页内资源全是绝对路径（`/assets/…`），`file://` 下会被解析到文件系统根 `file:///assets/...`，实测全部 `ERR_FILE_NOT_FOUND`（首页卡片 0 个、CSS 不生效）。现在全部改为相对路径 —— `index.html` 用 `assets/…`，静态工具页用 `../../assets/…`（由 `scripts/gen-static.js` 自动加前缀并自检层级）。
>
> **⛔ 新增资源时必须用相对路径**，别写 `/assets/…`；静态页的层级前缀由生成器负责，不要在工具代码里手写。
>
> `file://` 下有几处已知的行为差异（都已在 `app.js` 里处理，无需关心，但改路由时要记得）：
> - `history.pushState` 在 file 协议下抛 `SecurityError` → 自动退化为 hash 路由（`index.html#/tool/json/`）
> - 路由解析里 **hash 优先于 pathname**（静态页的 pathname 永远指向它自己那个工具）
> - 站内链接的 `href="/tool/x/"` 会被 Chrome 补上盘符成 `file:///D:/tool/x/`，点击拦截时要先剥掉 `/D:` 再匹配
> - 从静态工具页回首页仍要**真导航**到 `../index.html`（那里的静态页没有首页外壳）
...
**Q：双击 index.html 打开后是空白 / 没有样式？**
先确认你的仓库是最新的（2026-09-18 之前的版本资源是绝对路径，`file://` 下必然全 404，已在当天修好）。若仍异常，用方式 B/C 起本地服务器对照 —— 能在服务器下正常、只有 `file://` 异常，那就是又有人把某个资源写成了 `/assets/…` 绝对路径，跑 `node scripts/gen-static.js` 看自检会不会直接报出来。
...
- 用 `file://` 打开时自动退化回 hash 路由（`pushState` 在 file 协议下不可用），双击静态工具页也能认出自己是哪个工具
...
- **首页卡片也是真 `<a href="/tool/<id>/">`**（2026-09-18 从 `<div>` + onclick 改过来）：换来中键 / Ctrl+点击新开标签、右键复制链接、Tab 键聚焦回车打开，以及首页对内 155 条真实内链。星标因此只能用 `<span role="button">`（`<a>` 里不能再套交互元素），它的点击**必须 `preventDefault`** —— 光 `stopPropagation` 拦不住链接的默认跳转，点收藏会顺带把工具打开
...
3. **页内资源一律相对路径**（2026-09-18）—— 这是「方式 A：双击打开」能成立的前提，静态页的 `../../` 前缀由生成器统一加并自检。`app.js` 里为此有三处 file 协议专属处理（`pushState` 探测退化、hash 优先路由、点击拦截剥盘符），改动路由时别删。

### 改完怎么验

```bash
python -m http.server 8290
SITE=http://127.0.0.1:8290/ node scripts/verify-nav.js     # 导航 / 主题引导 / file:// 回归（33 项）
```

> `verify-nav.js` 覆盖四块：① 主题引导在 156 个页面里的引入与顺序、首屏主题与背景色、点同类工具链接不重载；② 四条「回首页」路径的完整性；③ 前进后退、搜索恢复、外链与修饰键放行、首页卡片的链接形态与星标不误跳；④ **`file://` 双击打开**（资源加载、静态工具页认得出自己、站内跳转不重载、返回拿到完整首页）。第 ④ 块不需要起服务器，直接读仓库文件。
...
1. **改过 `assets/` 或 `vendor/` 下任何文件后，同步更新 `?v=YYYYMMDD`**（当前为 `?v=20260918b`；同一天改多次就在后面加 `b`/`c`，别嫌丑）。`index.html` 与 `tool/*/index.html` 用的是同一个版本号，改完跑一次 `node scripts/gen-static.js` 即可让静态页自动跟随（版本号由脚本从 `index.html` 提取，不需要手改两处）。否则浏览器会继续吃旧缓存，出现「本地明明改了、线上没变」的假故障。
...
4. **新增任何页内资源时用相对路径**（`assets/…`，不要 `/assets/…`）。用了绝对路径，线上照常能跑、但「方式 A：双击打开」会整站崩，而且 `gen-static.js` 的自检会直接报「资源路径层级不对」并把退出码置 1。
...
就从「靠纪律」变成「不可能忘」。该脚本每次运行会先清空 `tool/` 再按注册表全量重建，并带自检（缺 h1、缺内链、`theme-boot.js` 没排在样式表之前、资源路径层级不是 `../../`，都会报错并置退出码非 0），可安全自动化。
